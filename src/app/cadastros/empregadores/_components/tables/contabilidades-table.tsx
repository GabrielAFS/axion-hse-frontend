...
import { getEmployerAccounting } from "../../fetch";
...
export default async function ContabilidadesTable({
  query,
  currentPage,
}: {
  query: string;
  ...
  const { accounting, totalPages } = await getEmployerAccounting(
    query,
    currentPage,
  );
      ...
      <Table
        ...
        renderRowsDesktop={(contact) => (
          <TableRow key={contact.id}>
            <TableCell>{contact.razaoSocial}</TableCell>
